...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.gateways = []
        self.gateways.append("192.168.1.254")
        self.gateways.append("192.168.2.254")
        self.gateways_MAC = []
        self.gateways_MAC.append("00:00:00:f1:f1:f1")
        self.gateways_MAC.append("00:00:00:f2:f2:f2")

        # ARP table at the controller level to manage gateways
        self.arp_table_gw = {}
        self.arp_table_gw["192.168.1.254"] = "00:00:00:f1:f1:f1"
        self.arp_table_gw["192.168.2.254"] = "00:00:00:f2:f2:f2"

        self.host_IP_to_MAC = {}
        self.host_IP_to_MAC["192.168.1.1"] = "00:00:00:11:11:11"
        self.host_IP_to_MAC["192.168.1.2"] = "00:00:00:12:12:12"
        self.host_IP_to_MAC["192.168.2.1"] = "00:00:00:21:21:21"
        self.host_IP_to_MAC["192.168.2.2"] = "00:00:00:22:22:22"
    ...
    def _packet_in_handler(self, ev):
        ...
        # save important information of the packet
        msg = ev.msg
        ...
        if eth.ethertype == ether_types.ETH_TYPE_LLDP:
            # ignore lldp packet
            return
        # save MAC address of source and destination
        dst = eth.dst
        src = eth.src
        ...
        self.logger.info(
            "packet in DPID=%s MAC_SRC=%s MAC_DST=%s IN_PORT=%s",
            dpid,
            src,
            dst,
            in_port,
        ...
        if dst in self.mac_to_port[dpid]:
            ...
            self.logger.info(
                "UNKNOWN destination MAC %s pkt is FLOODED %s", dst, out_port
            )

        # distinguish between ARP request and ARP reply
        # and also direct/indirect delivery
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            if arp_pkt.opcode == arp.ARP_REQUEST:
                ...
                # in case of direct delivery the gateway behave like a simple switch
                # it floods the packet to get the MAC address
                else:
                    self.logger.info(
                        "ARP REQUEST for host %s RECEIVED: direct delivery, out_port:%s",
                        target_ip,
                        out_port,
                    )
                    ...
                    if out_port != ofproto.OFPP_FLOOD:
                        match = parser.OFPMatch(
                            in_port=in_port, eth_type=0x0806, eth_dst=dst, eth_src=src
                        )
                        self.add_flow(datapath, 1, match, actions)
                    else:
                        data = None
                        ...
                        datapath.send_msg(out)
            # ARP reply
            # Need to check again if the target IP address belong to the gateway (direct/indirect delivery)
                ...
                if target_ip in self.gateways:
                    ...
                    # As soon as the gateway receive the ARP reply it has all the information needed to handle ICMP packet
                    # The gateway behave proactive and install new rules inside the switch
                    actions = []
                    # Change the source MAC address of the packet putting the MAC address of the gateway
                    actions.append(parser.OFPActionSetField(eth_src=arp_pkt.dst_mac))
                    # Change the destination MAC address of the packet by specifing the MAC address of the final destination
                    actions.append(parser.OFPActionSetField(eth_dst=arp_pkt.src_mac))
                    # Send the packet through the port where the ARP reply was received
                    ...
                    # direct delivery
                    self.logger.info(
                        "ARP REPLY for host %s RECEIVED, out_port %s: direct delivery",
                        target_ip,
                        out_port,
                    )
                    actions = [parser.OFPActionOutput(out_port)]
        ...
        # if ethernet carries IP parses the IP packet
            ...
            if ip_pkt.proto == inet.IPPROTO_ICMP:
                ...
                if (
                    ...
                    and dst in self.gateways_MAC
                    and ip_pkt.dst not in self.gateways
                ):
                    ...
                    # Controller has to install flow rule
                    match = parser.OFPMatch(
                        eth_type=0x0800, ip_proto=inet.IPPROTO_ICMP, ipv4_dst=ip_pkt.src
                    )
                    actions = []
                    ...
                    out_port = ofproto.OFPP_FLOOD
                    # Controller has to send an ARP reply, acting as the gateway
                    # obtain the MAC of dst IP  (in this case, GW_MAC - indirect delivery)
                    # ip_gw =  self.arp_table_gw.keys()[self.arp_table_gw.values().index(dst)]
                    ip_gw = list(self.arp_table_gw.keys())[
                        list(self.arp_table_gw.values()).index(dst)
                    ]
                    eth_pkt = pkt.get_protocol(ethernet.ethernet)
                    ether_hd = ethernet.ethernet(
                        src=eth_pkt.dst, ethertype=ether_types.ETH_TYPE_ARP
                    )
                    arp_hd = arp.arp(
                        hwtype=1,
                        proto=2048,
                    ...
                    datapath.send_msg(out)
                    self.logger.info(
                        "ARP REQUEST FROM GW %s to DST host %s sent", ip_gw, ip_pkt.dst
                    )
                elif (
                    icmp_pkt.type == icmp.ICMP_ECHO_REQUEST
                    and dst not in self.gateways_MAC
                    and ip_pkt.dst not in self.gateways
                ):
                    # direct delivery
                    self.logger.info(
                        "ECHO REQUEST to host %s: direct delivery!", ip_pkt.dst
                    )
                    actions = [parser.OFPActionOutput(out_port)]
                    # install a flow to avoid packet_in next time
                    if out_port != ofproto.OFPP_FLOOD:
                    ...
                    # direct delivery
                    self.logger.info(
                        "ECHO REPLY to host %s: direct delivery!", ip_pkt.dst
                    )
                    actions = [parser.OFPActionOutput(out_port)]
                    # install a flow to avoid packet_in next time
                    if out_port != ofproto.OFPP_FLOOD:
